...
def stream(ssc, pwords, nwords, duration):
    kstream = KafkaUtils.createDirectStream(
        ssc, topics = ['chatstream'], kafkaParams = {"metadata.broker.list": 'localhost:9092'})
    chats = kstream.map(lambda x: x[1])
    chats.pprint()
    # Each element of chats will be the text in dataset.
    # You need to find the count of all the positive and negative words in these chats.
    ...
    words = chats.flatMap(lambda line: line.split(" ")) #.filter(lambda x: (x in pwords) or (x in nwords))
